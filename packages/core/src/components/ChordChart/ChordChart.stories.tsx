    ...
    createTrack({
      measures: [
        ...
        createMeasure({
          number: 10,
          events: [
            createChord({ beat: 1, duration: 'whole', symbol: 'Gm', root: 'G', quality: 'minor' }),
          ],
        }),
        createMeasure({
          number: 11,
...
function InteractiveStory() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <ChordChart score={editor.score} editor={editor} onSelect={(sel) => setSelection(sel)} />
      <pre
        style={{
          fontSize: '0.8rem',
          background: '#f5f5f5',
      ...
      >
        {selection
          ...
          : 'Tab to or click a chord to see the selection here'}
      </pre>
    </div>
  )
}
...
function SelectionCallbacksStory() {
  const editor = useScore(autumnLeaves)
...
export const SelectionCallbacks: Story = {
  parameters: { layout: 'padded' },
  render: () => <SelectionCallbacksStory />,
}
